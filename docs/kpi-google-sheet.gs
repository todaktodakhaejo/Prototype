...
var HEADERS = [
  ...
  'ritualId', 'ritualCount', 'ritualSequence', 'isTextWritten', 'textLength',
...
var HEADER_NOTES = {
  ...
  'roundType': '세션 유형: full(의식 1개+) / write_only(글만, 의식 0) / ball_only(공놀이만) / abandoned(이탈).',
  ...
  'ritualId': '마지막으로 완료한 의식: burn(태우기)/shred(파쇄기)/plane(날리기)/jewelbox(보석함). 없으면 빈칸.',
  'ritualCount': '이 세션에서 완료한 의식 총 횟수(여러 번 가능).',
  'ritualSequence': '의식을 한 순서 (예: burn > tear > jewelbox).',
