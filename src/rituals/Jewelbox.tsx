...
// 내부 보석 자리(화면 좌표, top px). 새 보석은 가운데 빈자리에 꽂힌다.
const SLOTS = [
  { x: -58, y: 206 },
  { x: -30, y: 214 },
  { x: 30, y: 214 },
...
export default function Jewelbox({ text, onDone }: RitualProps) {
  ...
  const [storedColors] = useState(() => pickPastels(4))
  ...
  const showInside = open || stored // 뚜껑 열려 위에서 내부를 내려다보는 상태
  ...
  return (
    <div style={{ position: 'relative', width: 240, height: 300, touchAction: 'none' }}>
      ...
      <motion.div
        ...
        transition={{ duration: 2.6, delay: 3.4, times: [0, 0.05, 0.2, 0.38, 0.55, 0.75, 0.9, 1], ease: 'easeInOut' }}
        onAnimationComplete={() => {
          if (stored) finish()
        }}
      />
      ...
      {/* ===== 닫힌 보석함(정면 외관) — 드래그 전 ===== */}
      {!showInside && (
        <div style={{ position: 'absolute', left: '50%', bottom: 42, width: 178, height: 120, marginLeft: -89, zIndex: 2, borderRadius: 14, background: LEATHER, boxShadow: '0 22px 40px rgba(20,26,45,0.5), inset 0 2px 0 rgba(255,255,255,0.2), inset 0 -12px 24px rgba(0,0,0,0.42)', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', inset: 0, opacity: 0.5, background: GRAIN }} />
          {/* 윗광 */}
          <div style={{ position: 'absolute', left: 10, right: 10, top: 6, height: 12, borderRadius: 8, background: 'linear-gradient(180deg, rgba(255,255,255,0.22), rgba(255,255,255,0))' }} />
          {/* 뚜껑 경계선 */}
          <div style={{ position: 'absolute', left: 0, right: 0, top: 42, height: 2, background: 'rgba(0,0,0,0.3)', boxShadow: '0 1px 0 rgba(255,255,255,0.14)' }} />
          {/* 골드 잠금쇠 */}
          <div style={{ position: 'absolute', left: '50%', top: 36, width: 30, height: 20, marginLeft: -15, borderRadius: 4, background: GOLD, boxShadow: '0 2px 5px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.6)' }} />
        </div>
      )}

      {/* ===== 열린 보석함(위에서 내려다본 내부) — 드래그/담김 ===== */}
      {showInside && (
        <>
          {/* 뒤로 기대 선 뚜껑(가죽 + 크림 안감 + 목걸이 슬릿) */}
          <div
            style={{ position: 'absolute', left: '50%', bottom: 156, width: 196, height: 80, marginLeft: -98, zIndex: 1, transformOrigin: '50% 100%', transform: 'perspective(560px) rotateX(34deg)', borderRadius: '12px 12px 4px 4px', background: LEATHER, boxShadow: '0 -8px 18px rgba(20,26,45,0.4), inset 0 2px 0 rgba(255,255,255,0.16)', padding: 8 }}
          >
            <div style={{ position: 'absolute', inset: 8, borderRadius: 8, background: SATIN, boxShadow: 'inset 0 0 14px rgba(120,90,60,0.3)' }}>
          ...
          >
            <div style={{ position: 'absolute', inset: 0, borderRadius: 16, background: LEATHER, boxShadow: '0 22px 40px rgba(20,26,45,0.5), inset 0 2px 0 rgba(255,255,255,0.18)', overflow: 'hidden' }}>
              <div style={{ position: 'absolute', inset: 0, opacity: 0.5, background: GRAIN }} />
            </div>
            {/* 크림 새틴 내부 — 위가 좁은 사다리꼴(내려다보는 원근) */}
            <div style={{ position: 'absolute', left: 12, right: 12, top: 10, height: 96, background: SATIN, clipPath: 'polygon(8% 0, 92% 0, 100% 100%, 0 100%)', boxShadow: 'inset 0 12px 22px rgba(120,90,60,0.45)', overflow: 'hidden' }}>
      ...
      {stored && (
        <>
          ...
          <motion.div
            ...
            transition={{ duration: 3.3, times: [0, 0.16, 0.66, 0.88, 0.95, 1], ease: 'easeInOut' }}
          >
            <Gem />
          </motion.div>
          ...
          <motion.div
            style={{ position: 'absolute', left: '50%', top: NEW_SEAT_Y, width: 70, height: 70, marginLeft: -35, marginTop: -35, borderRadius: '50%', background: 'radial-gradient(circle, rgba(255,255,255,0.9) 0%, rgba(214,232,255,0.4) 40%, rgba(214,232,255,0) 70%)', zIndex: 6, pointerEvents: 'none' }}
            initial={{ scale: 0.2, opacity: 0 }}
            animate={{ scale: [0.2, 1.1, 1.5], opacity: [0, 0.85, 0] }}
